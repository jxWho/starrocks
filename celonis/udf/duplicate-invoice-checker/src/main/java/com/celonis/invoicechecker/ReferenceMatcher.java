...
public class ReferenceMatcher {
    ...
    public static Reference preprocess(Reference reference) {
    ...
    public final String[] process(String input) {
        ...
        for (int i = 0; i < jsonArray.length(); i++) {
            ...
            references.add(preprocess(reference));
        }
    ...
    static class Reference implements ClusterObjectInterface {
        ...
        private double computeSimilarity(Reference left, Reference right) {
            // Trie tree search.
            ...
            if (left.getModifiedReference().length() == right.getModifiedReference().length()) {
                ...
                if (left.getModifiedReference().length() > 3 && left.getCounters().equals(right.getCounters())) {
                    ...
                    for (int i = 0; i < left.getModifiedReference().length(); i++) {
                        ...
                        if (charLeft.equals(charRight)) {
                            ...
                            numMismatches++;
                            if (numMismatches >= maxErrors) {
                                return 0;
                            }
                        }
                    }
                    return 1 - numMismatches * 1.0 / maxErrors;
                }
            } else {
                ...
                StringPair strPair = getShortLongStr(left.getModifiedReference(), right.getModifiedReference());
                String longStr = strPair.getLongStr(), shortStr = strPair.getShortStr();
                int currMaxErrors = longStr.length() - shortStr.length();
                int numErrors = 0;
                int longIdx = 0;
                int numMatch = 0;
                for (int i = 0; i < shortStr.length(); i++) {
                    if (longIdx == longStr.length() || numErrors > currMaxErrors) {
                        break;
                    }
                    if (shortStr.charAt(i) == longStr.charAt(longIdx)) {
                        ...
                        numErrors++;
                        longIdx++;
                        while (numErrors <= currMaxErrors && longIdx < longStr.length() &&
                                shortStr.charAt(i) != longStr.charAt(longIdx)) {
                            longIdx++;
                            numErrors++;
                        }
                        if (numErrors > currMaxErrors || longIdx == longStr.length()) {
                            break;
                        } else {
                            numMatch++;
                            longIdx++;
