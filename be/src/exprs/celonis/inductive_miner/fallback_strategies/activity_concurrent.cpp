...
std::vector<activity_concurrent::cut_and_dfgs> activity_concurrent::compute_parallel_dfgs(
    const inductive_miner_config& miner_config, const directly_follows_graph& dfg, common::execution_context& context) {
  const auto num_vertices{boost::num_vertices(dfg)};
  std::vector<cut_and_dfgs> parallel_dfgs(num_vertices);
  std::ranges::transform(boost::make_iterator_range(boost::vertices(dfg)), begin(parallel_dfgs),
                         [&, num_vertices](auto par_vertex) {
                           ...
                           return cut_and_dfgs{par_cut, max_par_cut{}.apply_dfgs(miner_config, dfg, par_cut, context)};
                         });
...
std::optional<process_tree> activity_concurrent::apply_if_applicable(inductive_miner_config& miner_config,
  ...
  auto parallel_dfgs{compute_parallel_dfgs(miner_config, dfg, context)};
  ...
  const auto compute_result{[&]<typename IMPL>(IMPL /**/, cut_t cut, auto split) {
    ...
    process_tree pt{};
    auto sub_eventlogs{max_par_cut{}.apply_split(miner_config, dfg, split.cut, context)};
    ...
    if (empty_traces_base_case::is_applicable(remainder_dfg)) {
      ...
      pt = apply_cut_composite<IMPL>(cut, miner_config, remainder_dfg, context, miner_statistics);
    ...
    return process_tree{{process_tree::parallel{
        {{std::move(pt), inductive_miner_recurse(miner_config, single_activity_dfg, context, miner_statistics)}},
