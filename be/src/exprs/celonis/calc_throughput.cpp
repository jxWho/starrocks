...
int findActivity(const BinaryColumn& activity_data, const NullColumn::Container* activity_nulls, uint32_t begin_offset,
                 uint32_t end_offset, const Slice& name, Label label) {
...
StatusOr<ColumnPtr> CelonisCalcThroughputFunctions::celonis_calc_throughput(FunctionContext* context, const Columns& columns) {
    ...
    const Column* timestamp_array = columns[1].get();
    ...
    const BinaryColumn* activity_data = nullptr;
    ...
    if (auto nullable = dynamic_cast<const NullableColumn*>(activity_elements); nullable != nullptr) {
        activity_data = down_cast<const BinaryColumn*>(nullable->data_column().get());
    } else {
        activity_data = down_cast<const BinaryColumn*>(activity_elements);
    }
    ...
    auto start_activity = ColumnViewer<TYPE_VARCHAR>(columns[2]).value(0);
    auto end_activity = ColumnViewer<TYPE_VARCHAR>(columns[3]).value(0);
    ...
    for (size_t i = 0; i < num_cases; i++) {
        ...
        int end_activity_idx = findActivity(*activity_data, activity_nulls, activity_offsets_ptr[i],
