...
public:
     ...
     * not constant) and uses it to create an activity to int map which is used to encode the variants.
     * activity_to_index = {activity: idx for idx, activity in enumerate(activity_array)}
     * During encoding the variants, NULL activities are ignored. If an activity does not exist in the activity map, it
     * is mapped to -1. For example variant = ['B', 'A', NULL, 'A', 'W'], activity_array = ['A', 'B', 'C', 'D']
