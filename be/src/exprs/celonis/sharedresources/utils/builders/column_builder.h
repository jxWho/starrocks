 ...
 public:
  ...
  column_builder& size(const size_t size) {
    ...
    if (size_set_) {
      ...
      data_array_ = legacy_embedded_ctl::make_static_array_for_overwrite<T>(legacy_embedded_ctl::cast<size_t>(size), LEGACY_EMBEDDED_ALLOC_MSG(legacy_embedded_ctl::OUTPUT_COLUMN_MSG));
  ...
  column_builder& id(std::string new_id) {
    if (col_name_ == new_id) {
      throw std::runtime_error{
          "The ID is automatically set with name(). If you need to specify them individually, use name_only() "
          "and id_only()."};
    }
  ...
  memory::column_t build() {
    memory::column_t result =
        memory::builders::temp_column_builder(memory::col_name{col_name_}, memory::col_id{col_id_}, owner_, cache_key_)
            .create_from_data<T>(static_cast<row_id>(data_size_), std::move(data_array_), null_flags_, state_);
    reset();
    return result;
  }
...
class column_builder<cel_string_t> {
 ...
 private:
  ...
  string_buf_and_ptrs create_string_buffer() {
    ...
    auto ptrs{legacy_embedded_ctl::make_static_array_for_overwrite<cel_string_t>(legacy_embedded_ctl::cast<size_t>(data_size_),
    ...
    for (row_id i = 0; i < data_size_; ++i) {
      ...
      ptrs[i] = it->second;  // This is actually an offset; we correct it below once we have the final base address.
    }
