...
enum class CelonisTimeUnit { DAYS, WORKDAYS, HOURS, MINUTES, SECONDS, MILLISECONDS, INVALID };
...
static std::optional<TimestampValue> add_timeunits_helper(const TimestampValue& timestamp, CelonisTimeUnit time_unit,
                                                          int64_t add_value) {
...
public:
    ...
    std::optional<int64_t> remap_timestamp_ms(const TimestampValue& timestamp,
    ...
    std::optional<int64_t> millis_between(const TimestampValue& from_timestamp, const TimestampValue& to_timestamp,
    ...
    std::optional<TimestampValue> add_timeunits(const TimestampValue& timestamp, CelonisTimeUnit time_unit,
                                                int64_t add_value,
                                                const std::optional<std::string>& calendar_id) const {
        ...
        const std::vector<TimeRange>& time_ranges =
                is_workdays ? id_to_round_time_ranges_.find(calendar_id)->second : time_range_iter->second;
    ...
    std::optional<bool> is_timestamp_in(const TimestampValue& timestamp,
                                        const std::optional<std::string>& calendar_id) const {
        if (is_out_scope(timestamp, calendar_id)) {
            return std::nullopt;
        }
...
private:
    using IdToTimeRangesMap = phmap::flat_hash_map<std::optional<std::string>, std::vector<TimeRange>,
                                                   StdHash<std::optional<std::string>>>;
    using IdToWeekdayMap =
            phmap::flat_hash_map<std::optional<std::string>,
                                 phmap::flat_hash_map<int, celonis::accelerator::WeekdayCalendarEntry, StdHash<int>>,
                                 StdHash<std::optional<std::string>>>;
    ...
    void set_round_time_ranges() {
        ...
        for (const auto& kv : id_to_time_ranges_) {
            ...
            for (const auto& time_range : kv.second) {
                ...
                round_time_range.begin_ms =
                        floor_to_nearest_multiple(round_time_range.begin_ms, NUM_MILLISECONDS_PER_DAY);
    ...
    void set_no_weekly() {
        ...
        for (const auto& kv : id_to_time_ranges_) {
            id_to_no_weekly_.insert(
                    {kv.first, std::all_of(kv.second.begin(), kv.second.end(),
    ...
    IdToTimeRangesMap to_time_ranges(const celonis::accelerator::FactoryCalendar& factory_calendar) {
        ...
        for (size_t i = 0; i < factory_calendar.entries_size(); ++i) {
            ...
            if (entry.has_start_date() && entry.has_end_date() && entry.start_date() <= entry.end_date()) {
                const auto key = entry.has_calendar_id() ? std::make_optional(entry.calendar_id()) : std::nullopt;
        ...
        while (!calendars.empty()) {
            ...
            if (cur_id_to_time_ranges.has_value()) {
                if (id_to_time_ranges.has_value()) {
                    id_to_time_ranges =
                            intersect_id_to_time_ranges(id_to_time_ranges.value(), cur_id_to_time_ranges.value());
                } else {
    ...
    IdToTimeRangesMap to_time_ranges(const celonis::accelerator::WorkdayCalendar& workday_calendar) {
        phmap::flat_hash_map<std::optional<std::string>, std::map<int, std::bitset<366>>,
                             StdHash<std::optional<std::string>>>
                year_to_bitset_by_id;
    ...
    int64_t compute_duration(int64_t ms, const std::vector<TimeRange>& time_ranges,
    ...
    std::optional<int64_t> compute_overlap(int64_t left_ms, int64_t right_ms,
                                           const std::optional<std::string>& calendar_id,
                                           bool round_to_day = false) const {
        auto time_ranges_it = id_to_time_ranges_.find(calendar_id);
        if (time_ranges_it != id_to_time_ranges_.end()) {
            const auto& time_ranges =
                    round_to_day ? id_to_round_time_ranges_.find(calendar_id)->second : time_ranges_it->second;
            auto no_weekly_it = id_to_no_weekly_.find(calendar_id);
            DCHECK(no_weekly_it != id_to_no_weekly_.end());
            if (no_weekly_it->second) {
                auto cum_sum_it =
                        round_to_day ? id_to_round_cum_sum_.find(calendar_id) : id_to_cum_sum_.find(calendar_id);
    ...
    std::optional<TimestampValue> quick_add_ms(const TimestampValue& timestamp, int64_t ms, int64_t add_ms,
                                               const std::vector<TimeRange>& time_ranges,
    ...
    phmap::flat_hash_map<std::optional<std::string>, std::vector<int64_t>, StdHash<std::optional<std::string>>>
            id_to_cum_sum_;
    IdToTimeRangesMap id_to_round_time_ranges_;
    phmap::flat_hash_map<std::optional<std::string>, std::vector<int64_t>, StdHash<std::optional<std::string>>>
            id_to_round_cum_sum_;
    ...
    phmap::flat_hash_map<std::optional<std::string>, std::optional<Scope>, StdHash<std::optional<std::string>>>
            id_to_scope_;
};
...
static bool base64_encoded_string_to_calendar(const std::string& calendar_string,
...
static Status validate_multi_weekday_calendar(
        const celonis::accelerator::MultiWeekdayCalendar& multi_weekday_calendar) {
        ...
        if (has_calendar_id == -1) {
            ...
            if (has_calendar_id != weekday_calendar.has_calendar_id()) {
                return Status::InvalidArgument(
                        "In MultiWeekdayCalendar, ensure that the calendar_id is either set or not set in all "
                        "calendars.");
            }
        }
...
StatusOr<celonis::accelerator::Calendar> validate_and_to_proto(const std::string& calendar_string,
...
static StatusOr<std::optional<double>> timeunits_between(const TimestampValue& from_timestamp_raw,
                                                         const TimestampValue& to_timestamp_raw,
                                                         CelonisTimeUnit time_unit, const CalendarState& calendar_state,
                                                         std::optional<std::string>& calendar_id) {
    ...
    if (calendar_state.is_empty) {
        ...
        if (calendar_state.calendar.requires_calendar_id() && !calendar_id.has_value()) {
            ...
            milliseconds =
                    calendar_state.calendar.millis_between(from_timestamp, to_timestamp, calendar_id, round_to_day);
        }
    }
...
static StatusOr<std::optional<int64_t>> remap_timestamp_calendar(const TimestampValue& input_timestamp,
...
Status close(FunctionContext* context, FunctionContext::FunctionStateScope scope) {
    if (scope == FunctionContext::FRAGMENT_LOCAL) {
        auto* calendar_state =
                reinterpret_cast<CalendarState*>(context->get_function_state(FunctionContext::FRAGMENT_LOCAL));
...
StatusOr<ColumnPtr> CelonisTimeFunctions::millis_timestamp([[maybe_unused]] FunctionContext* context,
...
StatusOr<ColumnPtr> CelonisTimeFunctions::timestamp_millis([[maybe_unused]] FunctionContext* context,
                                                           const Columns& columns) {
    DCHECK_EQ(columns.size(), 1);
...
StatusOr<ColumnPtr> CelonisTimeFunctions::normalize_timestamp([[maybe_unused]] FunctionContext* context,
                                                              const Columns& columns) {
    DCHECK_EQ(columns.size(), 1);
    auto [all_const, num_rows] = ColumnHelper::num_packed_rows(columns);
    ColumnViewer timestamp_viewer = ColumnViewer<TYPE_DATETIME>(columns[0]);
    ...
    for (auto row = 0; row < num_rows; ++row) {
        ...
        result.append(timestamp_from_unix_millis(remap_timestamp_ms(timestamp)));
    }
    return result.build(all_const);
}
...
StatusOr<ColumnPtr> remap_timestamps_calendar_general([[maybe_unused]] FunctionContext* context,
    ...
    for (size_t row = 0; row < num_rows; ++row) {
        ...
        if (!config::treat_calendar_column_as_constant_in_calendar_functions || !calendar_state.has_value()) {
            ASSIGN_OR_RETURN(calendar_state, CalendarState::create_calendar_state(
                                                     calendar_array_column, row, false, false, time_unit_str,
...
static StatusOr<std::optional<bool>> timestamp_in_calendar(const TimestampValue& timestamp,
                                                           const CalendarState& calendar_state,
                                                           const std::optional<std::string>& calendar_id) {
    if (calendar_state.is_empty) {
        return std::nullopt;
...
StatusOr<ColumnPtr> get_calendar_entry_start_general([[maybe_unused]] FunctionContext* context,
                                                     const starrocks::Columns& columns) {
...
StatusOr<ColumnPtr> get_calendar_entry_start_const([[maybe_unused]] FunctionContext* context,
                                                   const starrocks::Columns& columns,
                                                   const CalendarState* calendar_state) {
    DCHECK_EQ(columns.size(), 3);
    auto [all_const, num_rows] = ColumnHelper::num_packed_rows(columns);
...
StatusOr<ColumnPtr> CelonisTimeFunctions::in_calendar(FunctionContext* context, const starrocks::Columns& columns) {
...
static StatusOr<celonis::accelerator::Calendar> get_calendar(const ColumnPtr& calendar_column, int row) {
    ASSIGN_OR_RETURN(const std::string calendar_string, get_calendar_string(calendar_column, row));
...
static StatusOr<std::optional<TimestampValue>> add_timeunits(const TimestampValue& timestamp, CelonisTimeUnit time_unit,
                                                             int64_t add_value, const CalendarState& calendar_state,
                                                             std::optional<std::string>& calendar_id) {
    if (calendar_state.is_empty) {
        if (calendar_id.has_value()) {
            return Status::InvalidArgument(
...
public:
    ...
    DateFilters(size_t row, const ColumnPtr& years_column, const ColumnPtr& quarters_column,
                const ColumnPtr& months_column, const ColumnPtr& weeks_column, const ColumnPtr& days_column) {
...
StatusOr<ColumnPtr> timeunits_between_calendar_general([[maybe_unused]] FunctionContext* context,
    ...
    for (size_t row = 0; row < num_rows; ++row) {
        ...
        if (!config::treat_calendar_column_as_constant_in_calendar_functions || !calendar_state.has_value()) {
            ASSIGN_OR_RETURN(calendar_state, CalendarState::create_calendar_state(
                                                     calendar_array_column, row, false, true, time_unit_str,
                                                     CalendarFunction::TIMEUNITS_BETWEEN_CALENDAR));
        }
        ASSIGN_OR_RETURN(const std::optional<double> diff, timeunits_between(from_timestamp, to_timestamp, time_unit,
                                                                             calendar_state.value(), calendar_id));
        if (diff.has_value()) {
...
static StatusOr<ColumnPtr> add_timeunits_calendar_const([[maybe_unused]] FunctionContext* context,
    ...
    for (size_t row = 0; row < num_rows; ++row) {
        ...
        const std::optional<TimestampValue> new_timestamp =
                calendar.add_timeunits(timestamp, time_unit, add_value, calendar_id);
        if (new_timestamp.has_value()) {
            result.append(new_timestamp.value());
        } else {
            result.append_null();
