...
[[nodiscard]] inline legacy_embedded_ctl::abstract_strategy_t extract_memory_tracking_strategy_from_context(
  ...
  return context_strategy == nullptr ? legacy_embedded_ctl::global_memory_tracking_strategy::get_global_memory_tracking_strategy()
...
[[nodiscard]] legacy_embedded_ctl::resource_owning_allocator<T> build_checked_allocator_for_memory_tracking_strategy(
    legacy_embedded_ctl::abstract_strategy_t memory_tracking_strategy, const legacy_embedded_ctl::utils::allocation_reason& reason,
    legacy_embedded_ctl::utils::allocation_priority priority, bool using_value_init, size_t minimum_size_for_memory_check,
    legacy_embedded_ctl::abstract_resource_t upstream_memory_resource) {
  legacy_embedded_ctl::abstract_resource_t tracking_resource{std::make_shared<legacy_embedded_ctl::tracking_memory_resource>(
      std::move(memory_tracking_strategy), std::move(upstream_memory_resource))};
  auto checked_resource{std::make_shared<legacy_embedded_ctl::checked_memory_resource>(reason, priority, minimum_size_for_memory_check,
...
[[nodiscard]] inline legacy_embedded_ctl::resource_owning_allocator<T> spawn_allocator(
    ...
    legacy_embedded_ctl::utils::allocation_priority priority = legacy_embedded_ctl::utils::allocation_priority::LOW, bool using_value_init = true,
    size_t minimum_size_for_memory_check = DEFAULT_MINIMUM_SIZE_FOR_MEMORY_CHECK_IN_BYTES,
    legacy_embedded_ctl::abstract_resource_t upstream_memory_resource = legacy_embedded_ctl::get_default_memory_resource()) {
...
[[nodiscard]] inline legacy_embedded_ctl::resource_owning_allocator<T> spawn_batched_allocator(
  ...
  auto batched_strategy{
      std::make_shared<legacy_embedded_ctl::memory::batched_memory_tracking_strategy>(std::move(context_strategy), min_batch_size)};
  return details::build_checked_allocator_for_memory_tracking_strategy<T>(
