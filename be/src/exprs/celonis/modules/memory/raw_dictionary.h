...
class raw_dictionary {
 public:
  ...
  [[nodiscard]] virtual dictionary_t convert_to_dictionary_t_release_data(const std::string& description) = 0;
...
class typed_raw_dictionary : public raw_dictionary {
 public:
  ...
  dictionary_t convert_to_dictionary_t_release_data(const std::string& description) final {
    auto handler = management::raw_data_handler<T>::create_data_handler(std::move(data_), description);
...
class typed_raw_dictionary<cel_string_t> : public raw_dictionary {
 public:
  ...
  [[nodiscard]] dictionary_t convert_to_dictionary_t_release_data(const std::string& description) final {
    const auto handler{management::string_data_handler::create_data_handler(
        std::move(data_), std::move(buffer_), management::pointer_data_handler_swap_type::SWAPPED_DICTIONARY,
        description)};
