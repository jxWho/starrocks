...
merge_result merge_n_dictionaries_raw(const std::vector<std::pair<dictionary_t, std::string>>& dictionaries,
  ...
  if (non_null_dict.empty()) {
    ...
    auto mapping_d1{ctl::make_static_array_for_overwrite<row_id>(dict_size, ALLOC_MSG(ctl::RAW_DATA_ALLOC_MSG))};
    std::iota(mapping_d1.begin(), mapping_d1.end(), row_id{0});

    auto mappings{ctl::make_static_array_for_overwrite<ctl::static_array<row_id>>(1, ALLOC_MSG(ctl::RETURN_VALUE_MSG))};
  ...
  auto mappings{
      ctl::make_static_array_for_overwrite<ctl::static_array<row_id>>(new_size, ALLOC_MSG(ctl::RAW_DATA_ALLOC_MSG))};
  ...
  for (std::size_t i{0}; i < new_size; ++i) {
    if (null_index_it != null_indexes.end() && i == *null_index_it) {
      auto mapping{ctl::make_static_array_for_overwrite<row_id>(1, ALLOC_MSG(ctl::RAW_DATA_ALLOC_MSG))};
      mapping[0] = 0;
