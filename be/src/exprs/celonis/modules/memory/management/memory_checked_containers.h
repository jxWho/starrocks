 ...
 * If there is not enough memory available the allocation throws with ctl::short_of_memory_exception.
 *
...
using checked_vector_t = std::vector<TYPE, std::scoped_allocator_adaptor<ctl::resource_owning_allocator<TYPE>>>;
constexpr size_t MIN_BYTES_STD_VECTOR_MEMORY_CHECK = 65536;

template <typename CONTAINER_TYPE,
          typename std::enable_if_t<ctl::is_base_of_template_v<std::vector, CONTAINER_TYPE>>* = nullptr>
...
using checked_ska_hash_map_t = ska::bytell_hash_map<
    ...
    std::scoped_allocator_adaptor<ctl::resource_owning_allocator<std::pair<KEY_TYPE, VALUE_TYPE>>>>;
constexpr size_t MIN_BYTES_SKA_HASHMAP_MEMORY_CHECK = 65536;

template <typename CONTAINER_TYPE,
          typename std::enable_if_t<ctl::is_base_of_template_v<ska::bytell_hash_map, CONTAINER_TYPE>>* = nullptr>
[[nodiscard]] auto checked_allocator(const common::execution_context& context,
                                     const ctl::utils::allocation_reason& reason,
                                     ctl::utils::allocation_priority priority = ctl::utils::allocation_priority::LOW) {
...
using checked_ska_hash_set_t =
    ska::bytell_hash_set<TYPE, HASH_FUNCTION, EQUALS_FUNCTION,
                         std::scoped_allocator_adaptor<ctl::resource_owning_allocator<TYPE>>>;
constexpr size_t MIN_BYTES_SKA_HASHSET_MEMORY_CHECK = 65536;

template <typename CONTAINER_TYPE,
          typename std::enable_if_t<ctl::is_base_of_template_v<ska::bytell_hash_set, CONTAINER_TYPE>>* = nullptr>
[[nodiscard]] auto checked_allocator(const common::execution_context& context,
                                     const ctl::utils::allocation_reason& reason,
                                     ctl::utils::allocation_priority priority = ctl::utils::allocation_priority::LOW) {
...
template <typename CONTAINER_TYPE,
          typename std::enable_if_t<ctl::is_base_of_template_v<std::unordered_set, CONTAINER_TYPE>>* = nullptr>
[[nodiscard]] auto checked_allocator(const common::execution_context& context,
                                     const ctl::utils::allocation_reason& reason,
                                     ctl::utils::allocation_priority priority = ctl::utils::allocation_priority::LOW) {
...
using checked_unordered_map =
    std::unordered_map<KEY, T, HASH_FUNCTION, EQUALS_FUNCTION,
                       std::scoped_allocator_adaptor<ctl::resource_owning_allocator<std::pair<const KEY, T>>>>;
constexpr size_t THRESHOLD_BYTES_UNORDERED_MAP_MEMORY_CHECK = 65536;

template <typename CONTAINER_TYPE,
          typename std::enable_if_t<ctl::is_base_of_template_v<std::unordered_map, CONTAINER_TYPE>>* = nullptr>
[[nodiscard]] auto checked_allocator(const common::execution_context& context,
                                     const ctl::utils::allocation_reason& reason,
                                     ctl::utils::allocation_priority priority = ctl::utils::allocation_priority::LOW) {
...
template <
    ...
    typename std::enable_if_t<ctl::is_base_of_template_v<boost::multi_index_container, CONTAINER_TYPE>>* = nullptr>
