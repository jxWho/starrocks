 ...
 */
[[nodiscard]] marking_t get_initial_marking(const bpmn_graph& model);
 ...
 */
[[nodiscard]] transitions_t get_enabled_vertex_transitions(const bpmn_graph& model, const marking_t& marking,
 ...
 */
[[nodiscard]] bool is_enabled(const bpmn_graph& model, const marking_t& marking, vertex_id_type vertex_id);

/*
 * For a given BPMN model and marking, returns all the transitions that can fire.
 */
[[nodiscard]] transitions_t get_enabled_transitions(const bpmn_graph& model, const marking_t& marking);
 ...
 */
[[nodiscard]] bool end_marking_reached(const bpmn_graph& model, const marking_t& marking);

/*
 * Checks if the given target vertex is reached by a token on any of the ingoing edges.
 */
[[nodiscard]] bool vertex_reached(const bpmn_graph& model, const marking_t& marking, vertex_id_type target);

/*
 * Checks if the given target vertex has a token on any of its ingoing edges.
 */
[[nodiscard]] bool vertex_reached_inverse(const bpmn_graph& model, const marking_t& marking, vertex_id_type target);

/**
 * Returns empty optional if the activity id is not present in the bpmn model
 */
[[nodiscard]] std::optional<vertex_id_type> get_vertex_id_for_task(const bpmn_graph& model, row_id activity_id);

// TODO(n.weber): Tests in case still needed in the future after rewrite (CPL-7268)
[[nodiscard]] bool has_enabled_gateway_transitions(const bpmn_graph& model,
