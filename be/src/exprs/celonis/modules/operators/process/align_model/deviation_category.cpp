...
deviation_categories_for_cases_t compute_categories(const alignments_t& alignments) {
  ...
  tbb::parallel_for(tbb::blocked_range<size_t>{0, alignments.size()},
                    [&deviation_categories = result, &alignments = std::as_const(alignments)](const auto& range) {
                      for (size_t idx_alignment{range.begin()}; idx_alignment < range.end(); ++idx_alignment) {
                        debug_assert(deviation_categories.at(idx_alignment).empty(),
                        ...
                        if (alignments.at(idx_alignment).has_value()) {
                          ...
                          deviation_categories.at(idx_alignment) =
                              ctl::make_static_array_for_overwrite<deviation_category>(
                                  alignment.size(), ALLOC_MSG(ctl::TEMPORARY_STORAGE_MSG));
                          // Iterate over moves in alignment
                          if constexpr (COMPUTE_V2 == compute_incomplete_category::NO) {
                            std::ranges::transform(alignment, deviation_categories.at(idx_alignment).begin(),
                                                   move_to_deviation_category_v1_fn{alignment_stats.get_counts()});
                          } else {
                            const auto indices{std::views::iota(size_t{0}, alignment.size())};
                            std::ranges::transform(alignment, indices, deviation_categories.at(idx_alignment).begin(),
...
template deviation_categories_for_cases_t compute_categories<compute_incomplete_category::YES>(
