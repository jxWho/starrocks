#include "compute_behavioral_relations.h"
...
void initialize_from_shortest_paths(behavioral_relations_matrix& behavioral_relations, petri_net_accessor& pn_accessor,
                                    const shortest_paths_matrix& shortest_paths) {
  for (const auto& pn_transition_i : pn_accessor.get_transitions()) {
    for (const auto& pn_transition_j : pn_accessor.get_transitions()) {
      ...
      const auto& distance_i_j{shortest_paths.get_distance(transition_i, transition_j)};
      const auto& distance_j_i{shortest_paths.get_distance(transition_j, transition_i)};
...
std::unordered_set<petri_net_transition_id, hash_transition> get_branch_transitions(
    petri_net_accessor& pn_accessor, petri_net_place_id starting_place, petri_net_transition_id join_transition) {
...
behavioral_relations_matrix compute_behavioral_relations(petri_net_accessor_with_parallel_sections& pn,
                                                         const shortest_paths_matrix& shortest_paths) {
  behavioral_relations_matrix behavioral_relations{pn.accessor().get_max_transition_id()};

  initialize_from_shortest_paths(behavioral_relations, pn.accessor(), shortest_paths);
