...
constraints_config_set optimal_alignment_set_cover::get_approximate_cover() && {
  ...
  for (auto it{std::max_element(begin(statistics_), end(statistics_), less_size)};
       it != end(statistics_) && !it->second.empty();
       it = std::max_element(begin(statistics_), end(statistics_), less_size)) {
    const auto max_set{std::move(it->second)};
    chosen_constraints.emplace_back(it->first);
    statistics_.erase(it);
    for (auto rit{begin(statistics_)}; rit != end(statistics_); /*update in body*/) {
