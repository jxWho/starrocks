...
shortest_paths_matrix::distance_type heuristic_min(shortest_paths_matrix::distance_type lhs,
                                                   shortest_paths_matrix::distance_type rhs) {
  return lhs == shortest_paths_matrix::UNCONNECTED   ? rhs
         : rhs == shortest_paths_matrix::UNCONNECTED ? lhs
                                                     : std::min(lhs, rhs);
}
...
[[nodiscard]] std::optional<shortest_paths_matrix::distance_type> transitions_estimate(
    FIRST first, LAST last, const marking_type& marking, const shortest_paths_matrix& metric,
    petri_net_accessor& petri_net) {
  const auto distance_min{[&metric, &petri_net, &marking](const auto& acc, auto transition) {
    auto min_value{std::min(acc, heuristic_to_transition{transition, metric, petri_net}.estimate(marking),
    ...
    if (min_value.has_value()) {
      ...
      min_value.value() += transition_weight;
    }
    return min_value;
  }};
  return std::accumulate(first, last, std::optional<shortest_paths_matrix::distance_type>{}, distance_min);
}
...
std::optional<heuristic_to_transition::cost_type> heuristic_to_transition::estimate(
  ...
  auto estimate{shortest_paths_matrix::UNCONNECTED};
  ...
  if (estimate == shortest_paths_matrix::UNCONNECTED) {
    return {};
  }
  return {estimate};
}
...
heuristic_to_markings::heuristic_to_markings(std::vector<petri_net::marking_type> targets,
                                             const shortest_paths_matrix& transition_distances,
                                             petri_net_accessor& petri_net)
