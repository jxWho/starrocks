...
struct fitting_prefix_heuristic {
  ...
  petri_net::petri_net_accessor& petri_net;
};
...
struct fitting_prefix_path_construction
  ...
  [[nodiscard]] auto get_filter(const marking_type& marking) const {
    const bool trace_is_not_done{marking.variant_place < trace_.size()};

    return [trace_is_not_done](const auto& transition) {
      bool should_remove{trace_is_not_done && (transition.is_log_move() || transition.is_visible_model_move())};
...
std::optional<trace_alignment> align_fitting_prefix(petri_net::petri_net_accessor& pn_accessor,
                                                    std::span<const row_id> variant,
                                                    const common::execution_context& context,
                                                    int max_trailing_model_moves, int iterations) {
  ...
  auto result{petri_net::a_star::a_star_search(synchronous_petri_net, fitting_prefix_heuristic{variant, pn_accessor},
                                               fitting_prefix_path_construction{variant, context},
                                               synchronous_petri_net.get_initial_marking(), max_trailing_model_moves,
                                               iterations)};
...
bool is_petri_net_run(petri_net::petri_net_accessor& pn_accessor_tt, const std::vector<row_id>& trace,
                      const common::execution_context& context, int max_iterations) {
  return align_fitting_prefix(pn_accessor_tt, trace, context, 0, max_iterations).has_value();
}
