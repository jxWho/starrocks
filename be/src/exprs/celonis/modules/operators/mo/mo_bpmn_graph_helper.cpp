...
void recalculate_counts(process_tree& pt) {
  std::visit(
      legacy_embedded_ctl::overloaded{[&](process_tree::exclusive& p) { p.recalculate_counts(); },
                                      [&](process_tree::sequence& p) { p.recalculate_counts(); },
                                      [&](process_tree::parallel& p) { p.recalculate_counts(); },
                                      [&](process_tree::redo& p) { p.recalculate_counts(); }, [&](auto& /*unused*/) {}},
      pt.node);
}

StatusOr<std::tuple<process_tree, process::inductive_miner_statistics, memory::column_t>> build(
    const std::string& process_tree_json, int id, const common::execution_context& parent_context) {
  ...
  for (rapidjson::SizeType i = 0; i < vertex_values.Size(); ++i) {
    ...
    switch (process_tree_type) {
      ...
      default:
        std::stringstream error;
        error << "celonis_mo_bpmn_graph: Process tree " << id << " has invalid process_tree_type " << process_tree_type
              << ".";
        return Status::InvalidArgument(error.str());
    }
  }
  for (rapidjson::SizeType i = edge_values.Size(); i > 0; --i) {
    ...
    RETURN_IF_ERROR(std::visit(
        legacy_embedded_ctl::overloaded{[&](process_tree::exclusive& p) {
          ...
          if (value.has_value()) {
            if constexpr (std::is_same_v<T, cel_string_t>) {
              objs[i].AddMember(rapidjson::StringRef(column_name),
                                rapidjson::Value().SetString(value.value(), allocator), allocator);
            } else {
            ...
            objs[i].AddMember(rapidjson::StringRef(column_name), rapidjson::Value(rapidjson::Type::kNullType),
                              allocator);
          }
          ++i;
        }
...
void add_column(memory::table_t table, const char* column_name, std::vector<rapidjson::Value>& objs,
  ...
  if (column->is_cel_string_type()) {
    ...
    LOG(ERROR) << "Not supported type of column with name [\"" << column_name << "\"] on table [\"" << table->get_name()
               << "\"].";
  }
}
...
std::string convert(const process::bpmn::bpmn_tables& bpmn_tables, const common::execution_context& context) {
  ...
  d.AddMember("bpmn_edges",
              convert_table(bpmn_tables.bpmn_edges, {"SOURCE_ID", "TARGET_ID", "OBJECT_ID", "OBJECT_COUNT"}, allocator,
                            context),
              allocator);
  d.AddMember("bpmn_nodes", convert_table(bpmn_tables.bpmn_nodes, {"NODE_ID", "NODE_TYPE"}, allocator, context),
              allocator);
  d.AddMember("bpmn_activities",
              convert_table(bpmn_tables.bpmn_activities, {"NODE_ID", "ACTIVITY_NAME"}, allocator, context), allocator);
  d.AddMember(
      "bpmn_model_descriptions",
      convert_table(bpmn_tables.bpmn_model_descriptions, {"OBJECT_ID", "BPMN_MODEL_DESCRIPTION"}, allocator, context),
      allocator);
  d.AddMember("bpmn_blocks",
              convert_table(bpmn_tables.bpmn_blocks, {"BLOCK_ID", "PARENT_BLOCK_ID", "OBJECT_ID", "BLOCK_TYPE"},
                            allocator, context),
              allocator);
  d.AddMember("bpmn_nodes_to_blocks",
              convert_table(bpmn_tables.bpmn_nodes_to_blocks, {"NODE_ID", "BLOCK_ID"}, allocator, context), allocator);
...
StatusOr<std::string> MoBpmnGraphHelper::execute(const std::vector<std::string>& process_trees_json) {
  ...
  auto mo_bpmn_graph_op =
      mo_bpmn_graph_operator(std::move(process_trees), std::move(statistics), std::move(activity_columns), context);
