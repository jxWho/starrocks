...
/**
 * @brief Produces and returns a JSON object (@see format/json/json.h) from the given meminfo instance
 */
format::json::json_t to_json(const full_meminfo& mi) {
  static constexpr auto UNIT = byte_unit::KiB;
  format::json::json_t json{};
...
format::json::json_object_t meminfo_change_json(const full_meminfo& old_meminfo, const full_meminfo& new_meminfo,
