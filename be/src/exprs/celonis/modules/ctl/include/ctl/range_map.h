...
class range_offsets {
 public:
  explicit range_offsets(static_array<IDX_TYPE>&& offsets) : offsets_{std::move(offsets)} {
    debug_assert(!offsets_.empty());
    debug_assert(offsets_[0] == 0);
  }
...
class range_map_builder {
 public:
  ...
  range_map_builder(range_map_histogram<IDX_TYPE>&& histogram, ELEMENT_TYPE default_value,
    ...
    debug_assert(!offsets_.empty() && offsets_[0] == 0);
  }
  ...
  void add_element(IDX_TYPE idx, ELEMENT_TYPE&& element) {
    debug_assert(static_cast<std::size_t>(idx) < offsets_.size() - 1);
    debug_assert(offsets_[idx] + filled_values_[idx] < offsets_[idx + 1]);
...
class ordered_range_map_builder {
 public:
  ...
  void add_element(ELEMENT_TYPE&& element) {
    debug_assert(static_cast<std::size_t>(curr_offset_) < values_.size());
