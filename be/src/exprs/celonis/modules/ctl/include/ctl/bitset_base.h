...
#include "ctl/bit.h"
#include "ctl/bits/dynamic_bitset_types.h"
...
inline DERIVED& bitset_crtp_base<PARALLELISM_SETTING, DERIVED>::experimental_atomic_set(
  ...
  debug_assert(!PARALLELISM_ENABLED);
  debug_assert(index < size());
  const auto [block_index, bit_index] = details::block_and_bit_index::get(index);
...
inline bool bitset_crtp_base<PARALLELISM_SETTING, DERIVED>::at(const bit_index_type index) const {
  if (index >= size()) {
    throw ctl::out_of_range{"Index [{}] is out of bounds for dynamic bitset of size [{}].", index, size()};
  }
...
bitset_crtp_base<PARALLELISM_SETTING, DERIVED>::data() const noexcept
  ...
  debug_assert(std::is_same_v<block_type, value_type>,
...
inline void bitset_crtp_base<PARALLELISM_SETTING, DERIVED>::apply_in_range_impl(CALLABLE&& callable,
  ...
  debug_assert(current_block_idx == to_block_idx);
  if constexpr (DO_FLIP) {
...
using bitset_crtp_base_t = ctl::bitset_crtp_base<ctl::details::bitset_types::parallelism_setting{false}, DERIVED>;

template <typename DERIVED>
using bitset_crtp_base_parallel_t =
    ctl::bitset_crtp_base<ctl::details::bitset_types::parallelism_setting{true}, DERIVED>;
