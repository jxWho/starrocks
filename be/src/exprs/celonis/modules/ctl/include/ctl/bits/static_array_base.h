...
#include "ctl/bits/memory_utils.h"
#include "ctl/bits/static_array_base_fwd.h"
#include "ctl/exception.h"
#include "ctl/memory/deregistering_deleter.h"
...
static_array_base<T, SHARED_T, ALLOCATOR_TYPE>::at(const index_type index) const {
  if (index >= size()) [[unlikely]] {
    throw ctl::out_of_range{"Index [{}] is out of bounds for static array of size [{}].", index, size()};
  }
  return data_[index];
}
...
inline void static_array_base<T, SHARED_T, ALLOCATOR_TYPE>::shrink(const size_t new_size) {
  ...
  if (new_size < size()) {
    auto new_data{
        make_array_internal(false, new_size, ALLOC_MSG("Allocation to shrink a static array"), get_allocator())};
...
static_array_base<T, SHARED_T, ALLOCATOR_TYPE>::make_array_copy_internal(
  ...
  if constexpr (IS_SHARED) {
    ...
    auto data_cpy{details::tracked_allocation<T, ALLOCATOR_TYPE, false /*DO_VALUE_INIT*/>(
        other.size(), ALLOC_MSG("Allocation for copy construction of a static array."),
