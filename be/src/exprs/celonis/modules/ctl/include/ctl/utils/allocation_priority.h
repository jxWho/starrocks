...
template <>
struct fmt::formatter<celonis::accelerator::ctl::utils::allocation_priority> {
  ...
  auto format(const celonis::accelerator::ctl::utils::allocation_priority& priority, FormatContext& ctx) {
    return fmt::format_to(ctx.out(), "{0}", celonis::accelerator::ctl::utils::to_string(priority));
  }
};
