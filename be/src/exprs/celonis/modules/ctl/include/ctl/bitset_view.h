...
class bitset_view : public bitset_crtp_base<PARALLELISM_SETTING, bitset_view<PARALLELISM_SETTING>> {
 ...
 public:
  ...
  bitset_view() = default;
  bitset_view(ctl::array_view<const block_type> view, size_t size) : view_{view}, size_{size} {
   ...
   */
  [[nodiscard]] ctl::array_view<const block_type> to_block_span() const noexcept { return view_; }

 private:
  ctl::array_view<const block_type> view_{};
  size_type size_{0};
};
...
class bitset_mutable_view : public bitset_crtp_base<PARALLELISM_SETTING, bitset_mutable_view<PARALLELISM_SETTING>> {
 ...
 public:
  ...
  bitset_mutable_view(ctl::array_view<block_type> view, size_t size) : view_{view}, size_{size} {
    common::runtime_assert(size_ <= view_.size() * base_type::BLOCK_SIZE,
   ...
   */
  [[nodiscard]] ctl::array_view<block_type> to_mutable_block_span() noexcept { return view_; }

 private:
  ctl::array_view<block_type> view_{};
  size_type size_{0};
};
