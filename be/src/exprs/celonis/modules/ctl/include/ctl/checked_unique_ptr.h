...
#include "ctl/bits/checked_ptr_utils.h"
...
template <typename T>
struct hash<celonis::accelerator::ctl::checked_unique_ptr<T>> {
  [[nodiscard]] constexpr size_t operator()(const celonis::accelerator::ctl::checked_unique_ptr<T>& p) const {
