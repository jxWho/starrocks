 ...
 * debug_assert(value == 1, "Value should be 1, but got {}", value);
 */
// NOLINTNEXTLINE(cppcoreguidelines-macro-usage)
#define debug_assert(...)                                                                                              \
  if constexpr (celonis::accelerator::ctl::IS_DEBUG_BUILD) {                                                           \
    celonis::accelerator::ctl::details::debug_assert_impl([] { return celonis::accelerator::ctl::source_location{}; }, \
                                                          __VA_ARGS__);                                                \
  }                                                                                                                    \
 ...
 */
// NOLINTNEXTLINE(cppcoreguidelines-macro-usage)
#define warning_assert(...)                                                                                           \
  celonis::accelerator::ctl::details::warning_assert_impl(celonis::accelerator::ctl::source_location{}, __VA_ARGS__); \
  static_assert(true, "Place a semicolon after warning_assert")
