 ...
 */
template <typename T>
struct is_cache_entry_unused<ctl::checked_shared_ptr<T>> {
  [[nodiscard]] bool operator()(const ctl::checked_shared_ptr<T>& value) const {
    // Less or equal one, since the cache owns an instance of the shared_ptr. So a use count of 1 means only the cache
    // still owns the value.
    return value.use_count() <= 1;
  }
 ...
 */
template <typename T>
struct is_cache_entry_unused<ctl::shared_static_array<T>> {
  [[nodiscard]] bool operator()(const ctl::shared_static_array<T>& value) const {
    // Less or equal one, since the cache owns an instance of the shared_ptr. So a use count of 1 means only the cache
    // still owns the value.
    return value.use_count() <= 1;
  }
...
class cache final {
 public:
   ...
   * @exception Throws valid exceptions thrown by the function.
   */
  [[nodiscard]] ctl::optional_ref<value_type> get_ignore_exception(const key_type& key) const;
