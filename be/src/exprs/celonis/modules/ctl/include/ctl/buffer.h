...
class buffer_allocation_node final {
 public:
  ...
  explicit buffer_allocation_node(size_type size, const allocator_type& allocator)
      : buffer_{ctl::make_static_array_for_overwrite<value_type>(size, ALLOC_MSG(BUFFER_ALLOC_MSG), allocator)} {}
...
class buffer {
 public:
  ...
  explicit buffer(const utils::allocation_reason& reason = ALLOC_MSG(node_t::BUFFER_ALLOC_MSG),
   ...
   */
  void copy_to_given_continuous_buffer_and_reset(ctl::array_view<value_type> target_buffer);
 ...
 private:
  ...
  void copy_to_given_continuous_buffer_unlocked(ctl::array_view<value_type> target_buffer) const;
...
inline void buffer<TYPE>::copy_to_given_continuous_buffer_and_reset(
    ctl::array_view<buffer<TYPE>::value_type> target_buffer) {
...
inline static_array<typename buffer<TYPE>::value_type> buffer<TYPE>::reallocate_to_continuous_buffer_unlocked() const {
  ...
  auto concatenated_buffer{make_static_array_for_overwrite<value_type>(
      size, ALLOC_MSG(node_t::BUFFER_ALLOC_MSG),
...
inline void buffer<TYPE>::copy_to_given_continuous_buffer_unlocked(
    ctl::array_view<buffer<TYPE>::value_type> target_buffer) const {
