...
// Copied from variant_stats_test.cpp with top variants and happy path stripped
...
protected:
    ...
    void match(const std::string& expected, const std::string& actual) {
        GraphResult gr;
        ASSERT_TRUE(gr.from_json(actual));
...
TEST_F(CelonisGraphTest, test_equality) {
    std::string s = R"json(
        {
           "dict":[
              {
    ...
    EXPECT_TRUE(gr.equals(gr));
    EXPECT_TRUE(vs1.equals(vs1));
    EXPECT_TRUE(gr.equals(vs1));
    EXPECT_TRUE(vs1.equals(gr));
}
...
TEST_F(CelonisGraphTest, test_large) {
    const AggregateFunction* func = get_aggregate_function("celonis_graph", TYPE_ARRAY, TYPE_VARCHAR, false);
...
TEST_F(CelonisGraphTest, test_enable_proto_encoding) {
    ...
    ctx->set_constant_columns({nullptr, nullptr, edge_count, enable_proto_encoding});
    auto state1 = ManagedAggrState::create(ctx, func);
    func->update_batch_single_state(ctx, col1->size(), raw_columns.data(), state1->state());
    ...
    std::cout << "\njson_string\n" << json_string.value() << "\n===\n";
    match(e_s, json_string.value());
}

TEST_F(CelonisGraphTest, test_enable_proto_encoding_empty) {
    const AggregateFunction* func = get_aggregate_function("celonis_graph", TYPE_ARRAY, TYPE_VARCHAR, false);
