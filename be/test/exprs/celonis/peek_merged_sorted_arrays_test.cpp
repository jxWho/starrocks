...
private:
    ...
    void Prepare() {
        std::vector<FunctionContext::TypeDesc> arg_types = {
                ...
                CelonisAnyValUtil::column_type_to_type_desc(TypeDescriptor::from_logical_type(TYPE_ARRAY)),
                CelonisAnyValUtil::column_type_to_type_desc(TypeDescriptor::from_logical_type(TYPE_ARRAY))};
        auto return_type = CelonisAnyValUtil::column_type_to_type_desc(TypeDescriptor::from_logical_type(InputLT));
...
TEST_F(CelonisPeekMergedSortedArraysTest, double_input_and_varchar_secondary_order) {
    ...
    EXPECT_EQ(2010.5, result->get(2).get_double());
    EXPECT_EQ(1010.5, result->get(3).get_double());
}
