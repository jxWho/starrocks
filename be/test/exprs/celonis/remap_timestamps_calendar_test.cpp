...
private:
    void Prepare() {
        std::vector<FunctionContext::TypeDesc> arg_types = {
                TypeDescriptor::from_logical_type(TYPE_DATETIME), TypeDescriptor::from_logical_type(TYPE_VARCHAR),
    ...
    StatusOr<ColumnPtr> Run() {
        ...
        result = CelonisTimeFunctions::remap_timestamps_calendar(
                ctx_.get(), {timestamp_column_, time_unit_column_, calendar_column_, calendar_id_column_});
        return result;
    }
    ...
    StatusOr<ColumnPtr> RunConstantCalendarAndTimeUnit(const std::vector<std::string>& calendar_strs,
    ...
    StatusOr<ColumnPtr> RunConstantCalendarAndTimeUnit() {
        ctx_->set_constant_columns({nullptr, ConstColumn::create(time_unit_column_, timestamp_column_->size()),
        ...
        const auto result =
                RunConstantCalendarAndTimeUnit(
                        {R"({"intersect_calendar": {"calendar1": {"multi_weekday_calendar": {)",
                         R"("calendars": {"thursday": {"use_day": true, "shift": {"begin": 28800000, "end": 61200000}}, "calendar_id": "id1"},)",
                         R"("calendars": {"thursday": {"use_day": true, "shift": {"begin": 0, "end": 61200000}}, "calendar_id": "id2"},)",
                         R"( }}, )", R"("calendar2": {"multi_weekday_calendar": {)",
                         R"("calendars": {"thursday": {"use_day": true, "shift": {"begin": 28800000, "end": 61200000}}},)",
                         R"(}})", R"(}})"},
                        "HOURS")
                        .value();
        ...
        const auto result =
                RunConstantCalendarAndTimeUnit(
                        {R"({"intersect_calendar": {"calendar1": {"factory_calendar": {)",
                         ...
                         R"("entries": {"start_date": 0, "end_date": 61200000, "calendar_id": "id2"}, )", R"( }}, )",
    ...
    }
    {
        ...
        const auto result =
                RunConstantCalendarAndTimeUnit(
                        {R"({"intersect_calendar": {"calendar1": {"multi_weekday_calendar": {)",
                         R"("calendars": {"thursday": {"use_day": true, "shift": {"begin": 28800000, "end": 61200000}}},)",
                         R"( }}, )", R"("calendar2": {"factory_calendar": {)",
                         R"("entries": {"start_date": 10000000, "end_date": 61200000, "calendar_id": "id1"}, )",
                         R"("entries": {"start_date": 0, "end_date": 61200000, "calendar_id": "id2"}, )", R"(}})",
                         R"(}})"},
                        "HOURS")
        ...
        const auto result =
                RunConstantCalendarAndTimeUnit(
                        {R"({"weekday_calendar": {)",
                         ...
                         R"("thursday": {"use_day": true, "shift": {"begin": 0, "end": 86400000} })", R"(} })"},
                        "DAYS")
                        .value();
        ASSERT_EQ(timestamp_column_->size(), result->size());
...
TEST_F(CelonisRemapTimestampsCalendarTest, weekday_calendar) {
    ...
    }
    {
        ...
        const auto result =
                RunConstantCalendarAndTimeUnit({first_half.c_str(), second_half.c_str()}, "SECONDS").value();
        ASSERT_EQ(timestamp_column_->size(), result->size());
        EXPECT_EQ(1L, result->get(0).get_int64());
    }
    {
        ...
        const auto result =
                RunConstantCalendarAndTimeUnit(
                        {R"({"weekday_calendar": {)",
                         ...
                         // use_day = false
                         R"("friday": {"use_day": false, "shift": {"begin": 28800000, "end": 61200000} })", R"(} })"},
                        "HOURS")
                        .value();
        ASSERT_EQ(timestamp_column_->size(), result->size());
        EXPECT_EQ(9L, result->get(0).get_int64());
        ...
        const auto result =
                RunConstantCalendarAndTimeUnit(
                        {R"({"weekday_calendar": {)",
                         // [8:00 am, 5:00 pm] = 9 hours
                         R"("thursday": {"use_day": true, "shift": {"begin": 28800000, "end": 61200000} }, )",
                         R"("friday": {"use_day": true, "shift": {"begin": 28800000, "end": 61200000} })", R"(} })"},
                        "HOURS")
                        .value();
        ASSERT_EQ(timestamp_column_->size(), result->size());
        ...
        const auto result =
                RunConstantCalendarAndTimeUnit(
                        {R"({"weekday_calendar": {)",
                         ...
                         R"("sunday": {"use_day": false, "shift": {"begin": 0, "end": 0} } )", R"(} })"},
                        "MINUTES")
                        .value();
        ASSERT_EQ(timestamp_column_->size(), result->size());
        ...
        const auto result =
                RunConstantCalendarAndTimeUnit(
                        {R"({"weekday_calendar": {)",
                         ...
                         R"("saturday": {"use_day": false, "shift": {"begin": 0, "end": 0} }, )",
                         // [9:00 am, 12:00 pm] = 3h
                         R"("sunday": {"use_day": true, "shift": {"begin": 32400000, "end": 43200000} } )", R"(} })"},
                        "HOURS")
                        .value();
        ASSERT_EQ(timestamp_column_->size(), result->size());
    ...
    }
    {
        ...
        const auto result =
                RunConstantCalendarAndTimeUnit(
                        {R"({"weekday_calendar": {)",
                         // [12:00 am, 8:00 am]
                         R"("wednesday": {"use_day": true, "shift": {"begin": 0, "end": 28800000} })", R"(} })"},
                        "SECONDS")
                        .value();
        ASSERT_EQ(timestamp_column_->size(), result->size());
...
TEST_F(CelonisRemapTimestampsCalendarTest, factory_calendar_without_id) {
    ...
    }
    {
        ...
        calendar_id_column_->append_datum(kNullDatum);
        const auto result = RunConstantCalendarAndTimeUnit(
                                    {R"({"factory_calendar": {)", R"("entries": {"start_date": 1, "end_date": 11}, )",
    ...
    }
    {
        ...
        const auto result =
                RunConstantCalendarAndTimeUnit(
                        {R"({"factory_calendar": {)", R"("entries": {"start_date": 1000, "end_date": 2000}, )",
                         R"("entries": {"start_date": 1200, "end_date": 1400}, )",
                         R"("entries": {"start_date": 100, "end_date": 200}, )", R"( }})"},
                        "MILLISECONDS")
                        .value();
        ASSERT_EQ(timestamp_column_->size(), result->size());
    ...
    }
    {
        ...
        calendar_id_column_->append_datum(kNullDatum);
        const auto result = RunConstantCalendarAndTimeUnit(
                                    {R"({"factory_calendar": {)", R"("entries": {"start_date": -11, "end_date": -1}, )",
                                     R"("entries": {"start_date": -40, "end_date": -20}, )",
                                     R"("entries": {"start_date": -300, "end_date": -20}, )", R"( }})"},
                                    "MILLISECONDS")
                                    .value();
        ASSERT_EQ(timestamp_column_->size(), result->size());
        ...
        const auto result =
                RunConstantCalendarAndTimeUnit(
                        {R"({"factory_calendar": {)",
                         ...
                         R"("entries": {"start_date": -300, "end_date": -20, "calendar_id": "id2"}, )", R"( }})"},
                        "MILLISECONDS")
                        .value();
        ASSERT_EQ(timestamp_column_->size(), result->size());
        ...
        const auto result =
                RunConstantCalendarAndTimeUnit(
                        {R"({"factory_calendar": {)",
                         ...
                         R"("entries": {"start_date": 5000, "end_date": 6000, "calendar_id": "id1"}, )", R"( }})"},
                        "SECONDS")
                        .value();
        ASSERT_EQ(timestamp_column_->size(), result->size());
        EXPECT_EQ(-1L, result->get(0).get_int64());
...
TEST_F(CelonisRemapTimestampsCalendarTest, workday_calendar_without_id) {
    {
        ...
        calendar_id_column_->append_datum(kNullDatum);
        const auto result =
                RunConstantCalendarAndTimeUnit({R"({"workday_calendar": )", R"({ "entries": { "year": 1970, )",
    ...
    }
    {
        ...
        const auto result =
                RunConstantCalendarAndTimeUnit({R"({"workday_calendar": )", R"({ "entries": { "year": 1970, )",
                                                celonis::get_workday_mask_str(365, {1}).c_str(), R"( } }})"},
                                               "HOURS")
                        .value();
        ASSERT_EQ(timestamp_column_->size(), result->size());
        EXPECT_EQ(0L, result->get(0).get_int64());
    ...
    }
    {
        ...
        const auto result =
                RunConstantCalendarAndTimeUnit({R"({"workday_calendar": {)", R"("entries": { "year": 1970, )",
                                                ...
                                                R"("entries": { "year": 1971, )",
                                                celonis::get_workday_mask_str(365, {5, 7}).c_str(), "},", R"( }})"},
                                               "DAYS")
                        .value();
        ASSERT_EQ(timestamp_column_->size(), result->size());
        EXPECT_EQ(8L, result->get(0).get_int64());
    }
    {
        ...
        calendar_id_column_->append_datum(kNullDatum);
        const auto result = RunConstantCalendarAndTimeUnit(
                                    {R"({"workday_calendar": {)", R"("entries": { "year": 1969, )",
                                     celonis::get_workday_mask_str(365, {0, 4, 10, 100, 360, 361, 362, 364}).c_str(),
                                     "},", R"("entries": { "year": 1970, )",
                                     celonis::get_workday_mask_str(365, {0, 1, 2}).c_str(), "},", R"( }})"},
                                    "DAYS")
                                    .value();
        ASSERT_EQ(timestamp_column_->size(), result->size());
        EXPECT_EQ(-4L, result->get(0).get_int64());
    }
    {
        ...
        calendar_id_column_->append_datum(kNullDatum);
        const auto result = RunConstantCalendarAndTimeUnit(
                                    {R"({"workday_calendar": {)", R"("entries": { "year": 1970, )",
                                     celonis::get_workday_mask_str(365, {0, 4, 10, 100, 150}).c_str(), "},",
                                     R"("entries": { "year": 1969, )",
                                     celonis::get_workday_mask_str(365, {0, 1, 10, 100, 150, 364}).c_str(), "},",
                                     R"("entries": { "year": 1968, )",
                                     celonis::get_workday_mask_str(366, {5, 7, 364, 365}).c_str(), "},", R"( }})"},
                                    "DAYS")
                                    .value();
        ASSERT_EQ(timestamp_column_->size(), result->size());
        EXPECT_EQ(-8L, result->get(0).get_int64());
...
TEST_F(CelonisRemapTimestampsCalendarTest, workday_calendar_with_id) {
    {
        ...
        const auto result =
                RunConstantCalendarAndTimeUnit(
                        {R"({"workday_calendar": {)", R"("entries": { "year": 1970, )",
                         celonis::get_workday_mask_str(365, {0, 4, 10, 100, 150}).c_str(), R"(, calendar_id: "id1"},)",
                         R"("entries": { "year": 1970, )",
                         celonis::get_workday_mask_str(365, {0, 4, 10, 100, 364}).c_str(), R"(, calendar_id: "id2"},)",
                         R"("entries": { "year": 1971, )", celonis::get_workday_mask_str(365, {5, 7}).c_str(),
                         R"(, calendar_id: "id1"},)", R"( }})"},
                        "DAYS")
                        .value();
        ASSERT_EQ(timestamp_column_->size(), result->size());
...
TEST_F(CelonisRemapTimestampsCalendarTest, malformed_workday_calendar) {
    {
        ...
        calendar_id_column_->append_datum("id");
        const auto result =
                RunConstantCalendarAndTimeUnit({R"({"workday_calendar": )", R"({ "entries": {"year": 1989,)",
    ...
    }
    {
        ...
        calendar_id_column_->append_datum("id");
        const auto result =
                RunConstantCalendarAndTimeUnit({R"({"workday_calendar": )", R"({ "entries": {)",
                                                celonis::get_workday_mask_str(365, {0, 2, 3}).c_str(), R"( } }})"},
                                               "HOURS");
        ASSERT_TRUE(result.status().is_invalid_argument());
    ...
    }
    {
        ...
        calendar_column_->append_datum(DatumArray{R"({"workday_calendar": )", R"({ "entries": {)",
                                                  celonis::get_workday_mask_str(365, {0, 2, 3}).c_str(), R"( } }})"});
        const auto result = Run();
        ASSERT_TRUE(result.status().is_invalid_argument());
        EXPECT_EQ(result.status().message(), "year is not set in a workday calendar entry.");
...
TEST_F(CelonisRemapTimestampsCalendarTest, non_const_calendar) {
    ...
    calendar_column_->append_datum(
            DatumArray{R"({"weekday_calendar": {)",
                       // [8:00 am, 5:00 pm]
                       R"("thursday": {"use_day": true, "shift": {"begin": 28800000, "end": 61200000} }, )",
                       R"("friday": {"use_day": false, "shift": {"begin": 0, "end": 0} })", R"(} })"});
    calendar_column_->append_datum(DatumArray{
            R"({"workday_calendar": {)", R"("entries": { "year": 1970, )",
            celonis::get_workday_mask_str(365, {0, 4, 10, 100, 150}).c_str(), R"(, calendar_id: "id1"},)",
            R"("entries": { "year": 1970, )", celonis::get_workday_mask_str(365, {0, 4, 10, 100, 364}).c_str(),
            R"(, calendar_id: "id2"},)", R"("entries": { "year": 1971, )",
            celonis::get_workday_mask_str(365, {5, 7}).c_str(), R"(, calendar_id: "id1"},)", R"( }})"});
    calendar_column_->append_datum(DatumArray{
            R"({"workday_calendar": {)", R"("entries": { "year": 1970, )",
            celonis::get_workday_mask_str(365, {0, 4, 10, 100, 150}).c_str(), R"(, calendar_id: "id1"},)",
            R"("entries": { "year": 1970, )", celonis::get_workday_mask_str(365, {0, 4, 10, 100, 364}).c_str(),
            R"(, calendar_id: "id3"},)", R"("entries": { "year": 1971, )",
            celonis::get_workday_mask_str(365, {5, 7}).c_str(), R"(, calendar_id: "id1"},)", R"( }})"});
    calendar_column_->append_datum(
            DatumArray{R"({"weekday_calendar": {)",
                       // [8:00 am, 5:00 pm]
...
TEST_F(CelonisRemapTimestampsCalendarTest, invalid_time_unit) {
    ...
    ASSERT_TRUE(result.status().is_invalid_argument());
    EXPECT_EQ(result.status().message(), "time unit must be one of WORKDAYS/DAYS/HOURS/MINUTES/SECONDS/MILLISECONDS.");
}
...
TEST_F(CelonisRemapTimestampsCalendarTest, malformed_calendar) {
    ...
    }
    {
        ...
        const auto result = RunConstantCalendarAndTimeUnit(
                {R"({"weekday_calendar": {)",
                 R"("unknown_day": {"use_day": true, "shift": {"begin": 0, "end": 1000} })", R"(} })"},
                "HOURS");
        ASSERT_TRUE(result.status().is_invalid_argument());
    ...
    }
    {
        ...
        calendar_column_->append_datum(DatumArray{
                ...
                R"("entries": { "year": 1970, )", celonis::get_workday_mask_str(365, {0, 4, 10, 100, 364}).c_str(),
                R"(, calendar_id: "id2"},)", R"("entries": { "year": 1971, )",
                celonis::get_workday_mask_str(365, {5, 7}).c_str(), R"(},)", R"( }})"});
        const auto result = Run();
        ASSERT_TRUE(result.status().is_invalid_argument());
        EXPECT_EQ(result.status().message(),
    ...
    const auto result =
            RunConstantCalendarAndTimeUnit(
                    {R"({"factory_calendar": {)",
                     ...
                     R"("entries": {"start_date": 1000, "end_date": 6000, "calendar_id": "id1"}, )", R"( }})"},
                    "MILLISECONDS")
                    .value();
    ASSERT_EQ(timestamp_column_->size(), result->size());
...
TEST_F(CelonisRemapTimestampsCalendarTest, invalid_weekday_calendar) {
    {
        ...
        const auto result = RunConstantCalendarAndTimeUnit(
                {R"({"weekday_calendar": {)",
                 R"("monday": {"use_day": true, "shift": {"begin": 0, "end": 1000} }, "calendar_id": "id" )", R"(} })"},
                "MILLISECONDS");
        ASSERT_TRUE(result.status().is_invalid_argument());
    ...
    }
    {
        ...
        calendar_id_column_->append_datum(kNullDatum);
        const auto result = RunConstantCalendarAndTimeUnit(
                {R"({"weekday_calendar": {)", R"("monday": {"use_day": true, "shift": {"begin": -1, "end": 1000} })",
                 R"(} })"},
                "MILLISECONDS");
        ASSERT_TRUE(result.status().is_invalid_argument());
    ...
    }
    {
        ...
        calendar_id_column_->append_datum(kNullDatum);
        const auto result = RunConstantCalendarAndTimeUnit(
                {R"({"weekday_calendar": {)", R"("monday": {"use_day": true, "shift": {"begin": 2000, "end": 1000} })",
                 R"(} })"},
                "MILLISECONDS");
        ASSERT_TRUE(result.status().is_invalid_argument());
    ...
    }
    {
        ...
        const auto result = RunConstantCalendarAndTimeUnit(
                {R"({"weekday_calendar": {)",
                 R"("monday": {"use_day": true, "shift": {"begin": 1000, "end": 86400005} })", R"(} })"},
                "MILLISECONDS");
        ASSERT_TRUE(result.status().is_invalid_argument());
        EXPECT_EQ(result.status().message(), "shift end is greater than 86400000 milliseconds in weekday calendar.");
    }
}
