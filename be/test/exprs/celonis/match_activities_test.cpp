...
TEST_F(CelonisMatchActivitiesTest, celonis_match_activities_string_data) {
    ...
    const auto result = CelonisMatchActivitiesFunctions::celonis_match_activities(
            ctx.get(),
            {array, other_filters, nodes_filter, other_filters, other_filters, other_filters, other_filters}).value();
...
TEST_F(CelonisMatchActivitiesTest, celonis_match_activities_with_excluding_nodes) {
    ...
    auto nodes_filter = ColumnHelper::create_column(TYPE_ARRAY_VARCHAR, true);
    nodes_filter->append_datum(DatumArray{"string1", "string3"});
    auto excluding_nodes_filter = ColumnHelper::create_column(TYPE_ARRAY_VARCHAR, true);
    excluding_nodes_filter->append_datum(DatumArray{"string2"});
    auto other_filters = ColumnHelper::create_column(TYPE_ARRAY_VARCHAR, true);
    other_filters->append_datum(DatumArray{});
    const auto result = CelonisMatchActivitiesFunctions::celonis_match_activities(
            ctx.get(), {array, other_filters, nodes_filter, other_filters, excluding_nodes_filter, other_filters,
                        other_filters}).value();
...
TEST_F(CelonisMatchActivitiesTest, celonis_match_activities_with_only_excluding_nodes) {
    ...
    other_filters->append_datum(DatumArray{});
    const auto result = CelonisMatchActivitiesFunctions::celonis_match_activities(
            ctx.get(), {array, other_filters, other_filters, other_filters, excluding_nodes_filter, other_filters,
                        other_filters}).value();
    EXPECT_EQ(4, result->size());
    EXPECT_EQ(0, result->get(0).get_int64());
    EXPECT_EQ(1, result->get(1).get_int64());
