...
private:
    ...
    void Run(const VariantRows& variant_rows, const std::string& bpmn_model_description_json,
        ...
        DeferOp close_fragment_local([&ctx] {
            CelonisAlignModelV2::align_model_v2_close(ctx.get(), FunctionContext::FRAGMENT_LOCAL);
        });
        ASSERT_OK(CelonisAlignModelV2::align_model_v2_prepare(
                ctx.get(), FunctionContext::FunctionStateScope::FRAGMENT_LOCAL));
        DeferOp close_thread_local([&ctx] {
            CelonisAlignModelV2::align_model_v2_close(ctx.get(), FunctionContext::THREAD_LOCAL);
        });
        ASSERT_OK(CelonisAlignModelV2::align_model_v2_prepare(ctx.get(), FunctionContext::FunctionStateScope::THREAD_LOCAL));
...
TEST_F(CelonisAlignModelV2Test, InvalidModel) {
    ...
    ASSERT_OK(CelonisAlignModelV2::align_model_v2_prepare(ctx.get(), FunctionContext::FunctionStateScope::FRAGMENT_LOCAL));
    ASSERT_OK(CelonisAlignModelV2::align_model_v2_prepare(ctx.get(), FunctionContext::FunctionStateScope::THREAD_LOCAL));
    ...
    ASSERT_OK(CelonisAlignModelV2::align_model_v2_close(ctx.get(), FunctionContext::FunctionStateScope::FRAGMENT_LOCAL));
}
