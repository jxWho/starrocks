...
TEST_F(CelonisTimeFunctionsTest, date_match_prepare) {
    {
        auto timestamps = ColumnHelper::create_column(TypeDescriptor(TYPE_DATETIME), false);
        ...
        const auto num_rows = timestamps->size();
        ...
        auto utils = std::make_shared<FunctionUtils>();
        auto const_years = ConstColumn::create(years, num_rows);
        auto const_quarters = ConstColumn::create(quarters, num_rows);
        auto const_months = ConstColumn::create(months, num_rows);
        auto const_weeks = ConstColumn::create(weeks, num_rows);
        auto const_days = ConstColumn::create(days, num_rows);
        utils->get_fn_ctx()->set_constant_columns(
                {nullptr, const_years, const_quarters, const_months, const_weeks, const_days});
        utils->get_fn_ctx()->_arg_types.emplace_back(FunctionContext::TypeDesc{TYPE_DATETIME});
        utils->get_fn_ctx()->_arg_types.emplace_back(FunctionContext::TypeDesc{TYPE_ARRAY});
        utils->get_fn_ctx()->_arg_types.emplace_back(FunctionContext::TypeDesc{TYPE_ARRAY});
        utils->get_fn_ctx()->_arg_types.emplace_back(FunctionContext::TypeDesc{TYPE_ARRAY});
        ...
        utils->get_fn_ctx()->_arg_types.emplace_back(FunctionContext::TypeDesc{TYPE_ARRAY});
        // prepare
        ASSERT_TRUE(CelonisTimeFunctions::date_match_prepare(utils->get_fn_ctx(),
                                                             FunctionContext::FunctionStateScope::FRAGMENT_LOCAL).ok());
        // execute
        const auto result = CelonisTimeFunctions::date_match(utils->get_fn_ctx(),
                                                             {timestamps, years, quarters, months, weeks, days
                                                             }).value();
        ASSERT_EQ(timestamps->size(), result->size());
        EXPECT_EQ(1L, result->get(0).get_int64());
        EXPECT_EQ(1L, result->get(1).get_int64());
        ...
        // close
        ASSERT_TRUE(CelonisTimeFunctions::date_match_close(utils->get_fn_ctx(),
                                                           FunctionContext::FunctionStateScope::FRAGMENT_LOCAL).ok());
    }
    // NULL filters
    {
