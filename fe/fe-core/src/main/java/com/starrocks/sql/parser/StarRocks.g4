...
aggregationFunction
    ...
    | MULTI_ARRAY_AGG '(' setQuantifier? expression (',' expression)* (ORDER BY sortItem (',' sortItem)*)? ')'
    | CELONIS_SORTED_FIRST '(' setQuantifier? expression (ORDER BY sortItem (',' sortItem)*)? ')'
    | CELONIS_SORTED_LAST '(' setQuantifier? expression (ORDER BY sortItem (',' sortItem)*)? ')'
...
windowFunction
    ...
    | name = RANK '(' ')'
    | name = DENSE_RANK '(' ')'
    | name = CELONIS_LINEAR_INTERPOLATE '(' expression ')'
...
nonReserved
    ...
    | CELONIS_SORTED_FIRST | CELONIS_SORTED_LAST
